...
// Caller must have already created a new thread for us, and must close the thread when we return.
{
	...
	ResultType result = aVariant.mCallback->ExecuteInNewThread(g_script.mThisHotkeyName, &aParams, 1);
...
ResultType Hotkey::Dynamic(LPTSTR aHotkeyName, LPTSTR aLabelName,
...
// Returns OK or FAIL.
{
	...
	default: // aHookAction is 0 or an AltTab action.  COMMAND: Hotkey, Name, Label|AltTabAction
		...
		else // Hotkey already exists (though possibly not the required variant).  Update the hotkey if appropriate.
		{
			...
			// If the above changed the action from an Alt-tab type to non-alt-tab, there may be a callback present
			...
			{
				// If there's a matching variant, update it's callback. Otherwise, create a new variant.
				if (variant) // There's an existing variant...
				{
					if (aCallback != variant->mCallback) // ...and it's callback is being changed.
					{
...
// is created, the aCallback's destination line is not yet known.  So the aCallback is used a placeholder.
...
// points to an implicit hotkey function).  The only time aCallback should
...
Hotkey::Hotkey(HotkeyIDType aID, IObject * aCallback, HookActionType aHookAction,
 LPTSTR aName, bool aSuffixHasTilde)
// Constructor.
// Caller provides aCallback rather than a Line* because at the time a hotkey or hotstring
// is created, the callbacks's destination line is not yet known.  So aCallback is used a placeholder.
...
// Even if aJumpToLabel is NULL, a non-NULL mCallback will be stored in each variant so that
...
// The caller is responsible for calling ManifestAllHotkeysHotstringsHooks(), if appropriate.
{
	...
	v.mCallback = aJumpToLabel ? aJumpToLabel : g_script.mPlaceholderLabel;
...
// we return.
{
	...
	result = mCallback->ExecuteInNewThread(g_script.mThisHotkeyName, &aParams, 1);
...
ResultType Hotstring::AddHotstring(LPTSTR aName, IObjectPtr aCallback, LPTSTR aOptions,
 LPTSTR aHotstring, LPTSTR aReplacement, bool aHasContinuationSection, UCHAR aSuspend)
// Caller provides aCallback rather than a Line* because at the time a hotkey or hotstring
// is created, the callback's destination line is not yet known.  So aCallback is used a placeholder.
// Returns OK or FAIL.
...
// (e.g. "ahk"), which does not have to be unique, unlike the combination of aHotstring and aOptions
// for the current g->HotCriterion.
...
Hotstring::Hotstring(LPTSTR aName, IObjectPtr aCallback, LPTSTR aOptions, LPTSTR aHotstring,
...
BIF_DECL(BIF_Hotstring)
{
	...
	if (existing)
	{
		...
		{
			IObjectPtr new_callback = action_obj ? action_obj : g_script.mPlaceholderLabel; // Other parts may rely on mCallback always being non-NULL.
			...
			if (new_callback != existing->mCallback)
			{
				existing->mCallback = new_callback;
			}
		}
